            ...
            if(array[i] == i){
                if(array[i+1] == i+1){
                    if(array[i+2] == i+2){
                        if(array[i+3] == i+3){
                            if(array[i+4] == i+4){
                                return 1;         // Linear victory (only victory condition I have working)
                            }
                        }
                    }
                }
...
int main()
{
    ...
    vector<char> BoardC;
    for(int i = 0; i < BOARDSIZE+1; i++)    BoardC.push_back(sBoard[i]);
    //for(auto itr = BoardC.begin(); itr != BoardC.end(); itr++) cout << *itr;
